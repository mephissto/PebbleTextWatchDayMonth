...
#define MY_UUID { 0xA5, 0xCA, 0x2F, 0x44, 0x6E, 0x4B, 0x43, 0xFA, 0x8B, 0xC8, 0x83, 0x12, 0x4B, 0xE8, 0x65, 0xB2 }
PBL_APP_INFO(MY_UUID,
             "Text with day month", "Mephissto",
             1, 0,
...
void updateDate(PblTm *t)
{
  //strcpy(dateStr, WEEKDAYS[t->tm_wday]);
  //strcat(dateStr, " ");
  strcpy(dateStr, DAYS[t->tm_mday]);
  strcat(dateStr, " ");
  strcat(dateStr, MONTH[t->tm_mon]);
}
...
void display_time(PblTm *t)
{
  ...
  if(strcmp(dateLayer.text, dateStr) != 0) {
        text_layer_set_text(&dateLayer, dateStr);
  }
}
...
void handle_init(AppContextRef ctx) {
  ...
  // date layer
  text_layer_init(&dateLayer, GRect(0, 147, 144, 20));
  configureDayOfMonth(&dateLayer);
